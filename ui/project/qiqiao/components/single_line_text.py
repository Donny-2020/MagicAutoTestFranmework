...
from ui.project.qiqiao.element.single_line_text import SingleLineTextElement

class SingleLineText(Driver):

    @retry(stop_max_attempt_number=3, wait_fixed=2.5)
    def sendValueToLineText(self,fild_name,value):
        '''单行文本输入值'''
        loc = SingleLineTextElement().sendValueLoc.format(name=fild_name)
        self.sendKeys(loc,value)

    @retry(stop_max_attempt_number=3, wait_fixed=2.5)
    def searchDataByLineText(self,filed_name,value):
        '''使用单行文本字段筛选页面数据'''
    ...
    @retry(stop_max_attempt_number=3, wait_fixed=2.5)
    def sendValueToLineTextInSubForm(self,sub_form,filed_name,value):
        '''子表单的单行文本输入数据'''
