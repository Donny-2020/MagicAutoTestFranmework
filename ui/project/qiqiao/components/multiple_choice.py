...
class MultipleChoice(Driver):
    ...
    def sendValueToMultipleChoice(self,fild_name,value):
        '''多项选择输入值'''
        loc = MultipleChoiceElement().sendValueLoc.format(name=fild_name)
        self.sendKeys(loc,value)

    @retry(stop_max_attempt_number=3, wait_fixed=3000)
    def searchDataByMultipleChoice(self,filed_name,value):
        '''使用多项选择字段筛选页面数据'''
    ...
    @retry(stop_max_attempt_number=3, wait_fixed=3000)
    def sendValueMultipleChoiceInSubForm(self,sub_form,filed_name,value):
        '''子表单的多项选择输入数据'''
